...
public class String_of_Beads {
    ...
    public boolean hasAdjacentMotifs(){
        if (motifs.size() < 2) return false;
        Motif m1, m2;
        for (int i=0; i < (motifs.size()-2); i++) {
            m1 = motifs.get(i);
            m2 = motifs.get(i+1);
            if (m2.inferred_start == m1.inferred_end+1) return true;
        }
        return false;
    }
    ...
    public boolean hasDYW(){
        ...
        for (int i=0; i < motifs.size(); i++) {
            m = motifs.get(i);
            if (m.type.startsWith("DYW") || m.type.equals("KPAKA")) return true;
        }
        return false;
    }
    
    public ArrayList getSubstring() {
    ...
    public void inferMotifs(PPR ppr) {
        ...
        do { 
            ...
            for (int i=motifs.size()-1; i >= 0; i--) {
                ...
                //System.out.println("inferring motif start: " + m2.type + "\t" + String.valueOf(m2.inferred_start));
                m2.inferStart(limit);
                
                
                if (m2.inferred_start > m2.alignment_start) {
                    System.out.println("inferred start exceeds alignment start for " + ppr.getID() + " motif " + String.valueOf(i+1));
                }
            }
        } while (overlap);
        ...
        for (int i=0; i < motifs.size(); i++) {
            ...
            if (limit < m1.inferred_start){
                System.out.println("limit precedes inferred start for " + ppr.getID() + " motif " + String.valueOf(i+1));
            }
            ...
            if (i+1<motifs.size()) {
                ...
                if (m1.inferred_start+Motif.MAX_MOTIF_LENGTH > m2.inferred_start) {
                    ...
                    //System.out.println("inferring motif end: " + m1.type + " " + String.valueOf(m1.inferred_end) + " with respect to motif " + m2.type + " starting at " + String.valueOf(m2.inferred_start) );
                }
            }
            ...
            if (!define_end_by_start_of_next_motif) {
                ...
                switch(m1.getType()) {
                    case "P": case "P1": case "P2": case "S1": case "S2": case "SS": case "L1": case "L2": case "E1": case "E2":
                        limit = Math.min(limit,m1.inferred_start+canonical_length-1);
                        break;
                    case "E+": case "DYW": case "KPAKA":
